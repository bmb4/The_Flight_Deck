...
def getHandler(self, request):
    ...
    if path == "":
        ...
        content = content.replace('{{ Wins 3 }}', leaders[2][1])
        content = content.replace('{{ Username 1 }}', leaders[0][0])
        content = content.replace('{{ Username 2 }}', leaders[1][0])
        content = content.replace('{{ Username 3 }}', leaders[2][0])
        return responses.create200(content, "text/html", len(content))
    elif path == "static":
